...
本檔只定義 M4 Core 整合的 early memory preflight。`M4-REG-001` 只在昂貴
integration loop、quality 或 soak 前，以相同 target 的最小 smoke command 排除明顯
4GB 容量風險；它不是 milestone gate，也不取代 M4A/M4B 的 exact-SHA acceptance。
