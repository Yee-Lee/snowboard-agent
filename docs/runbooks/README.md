...
| **M4 起共用 Candidate Gate** | [`candidate_hardware_gate.md`](candidate_hardware_gate.md) | **Portable-first 與單一 frozen SHA**<br>(Python matrix, preflight, debug / acceptance isolation, evidence identity) | 3.11 / 3.12 / 3.13 portable matrix + 單一部署 runtime硬體驗收 |
...
5. **候選先於硬體**：自 M4 起先完成 portable matrix與candidate freeze；Pi debug evidence不能混入正式acceptance bundle。
