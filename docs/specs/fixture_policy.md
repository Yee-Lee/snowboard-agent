...
Status: `AUTHORITATIVE / BASELINE FROZEN AT AR1M0 / SCHEDULE REVISED AT AR1M1 EXIT`
...
- AR1M1 audits historical sources before real smoke, freezes one authorized
  approximately three-second PCM smoke fixture, completes the metadata coverage
  matrix, and documents every gap and minimum closure action. It collects a
  smoke replacement only if no suitable reusable item exists.
- At AR1M2 entry and before any formal execution, collect or derive only the
  minimum authorized prerecorded gap fixtures or annotations and complete their
  audits. User then reviews holdout proposals and all four disjoint role
  manifests are frozen.
- After AR1M2 formal execution begins, AR1M2 cannot add result-driven fixtures
  or inspect final holdout.
...
The scheduling change is recorded in
`asr_r1/manifests/m1_fixture_schedule_revision.json`. It was approved before
AR1M2 formal scoring and changes only the milestone boundary: coverage,
authorization, prior-use, role-separation, and fail-closed requirements remain
unchanged.
