...
- Status: PHASE 1 COMPLETE / PHASE 2 RECEIPT ACKNOWLEDGED / COMMIT REVIEW PENDING / CLEANUP NOT AUTHORIZED
...
Do not create `PI_CORE_DEV_WORKSPACE`, a bare repository, new Pi worktrees,
`PI_PROD_RELEASE_ROOT`, `PI_PROD_CONFIG_ROOT`, `PI_PROD_STATE_ROOT`, service
accounts, permissions, or activation pointers. Core owns that Phase 2
provisioning and will return a workspace receipt before Audio begins Phase 3 work
on the new layout.
...
All Pi paths in this request are logical locators. Their host-specific root
mapping must remain outside Git in the controlled environment.

Until Core switches to a verified canonical deployment, do not move or remove the
currently referenced M4A product under
`PI_M4A_RUNS/6c3ba95-20260829-dev01/product`. Preserve the accepted Audio
...
2. Inventory the following roots by logical owner, unique bytes, reproducibility,
   ...
   - `PI_AUDIO_WORKSPACE`;
   - `PI_AUDIO_PRIVATE`;
   - `PI_M4A_PRODUCTS` Audio runtime closure;
   - `PI_M4A_RUNS`;
