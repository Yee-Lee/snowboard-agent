...
| --- | --- | --- |
| [M2 Gate review request](../../poc_audio/deliveries/REQ-AUDIO-M2-GATE-REVIEW-001.md) | `RESPONSE RECEIVED / M2 BLOCKED` | Reviewer 已接受 ASR/TTS disposition，正式授權 WebRTC/Silero 並固定 VAD profile/gates。尚缺 VAD bounded scorecard、M3 HAL SHA 與 retest packet；見 [`RESP-AUDIO-M2-GATE-REVIEW-001`](../reviews/RESP-AUDIO-M2-GATE-REVIEW-001.md)。 |
...
| [VAD execution scope change request](../../poc_audio/deliveries/CR-AUDIO-M4A-G1B-VAD-SCOPE-001.md) | `AUTHORIZED / EXECUTION PENDING` | Reviewer 已接受 WebRTC 2.0.10 primary、Silero 6.2.1 conditional fallback，固定 level 3、300/500 ms padding、start/end recall 95%/90% 與既有 boundary/false-start gates；只允許 bounded scorecard。 |
