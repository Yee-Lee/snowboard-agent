...
| --- | --- | --- |
| [Corrected M2 Gate closure request](../../poc_audio/deliveries/REQ-AUDIO-M2-GATE-CLOSURE-002.md) | `READY FOR REVIEW / USER PUBLICATION CONFIRMED` | 舊 VAD no-go 已因方法缺陷撤回；corrected evidence 提出 Silero conditional M3 finalist、低音量 target-mic blocker 與 corrected M3 entry lock。請 Reviewer/Designer 關閉 findings 或只列 exact remaining blocker。 |
...
| [M2 Gate review request](../../poc_audio/deliveries/REQ-AUDIO-M2-GATE-REVIEW-001.md) | `RESPONSE ACTIONED / CORRECTED CLOSURE REQUESTED` | Reviewer 接受 ASR/TTS 並授權 VAD；修正後 VAD evidence 與 M3 entry lock 已提交至 closure 002。原 response 見 [`RESP-AUDIO-M2-GATE-REVIEW-001`](../reviews/RESP-AUDIO-M2-GATE-REVIEW-001.md)。 |
