...
| --- | --- | --- |
| [M4 runtime-closure request](../../poc_audio/deliveries/REQ-AUDIO-M4-RUNTIME-CLOSURE-002.md) | `BLOCKING ACTION REQUIRED — CORE INTEGRATION MUST NOT START` | 002 取代 001，要求 Core 在整合前完成 Core HAL/controller 與三 finalist 的 separated isolated offline closure；必須固定 `pyalsaaudio`、`samplerate`、PyYAML、NumPy、wheel checksum 與 fail-closed clean preflight。未完成不得開始 Gate 3 integration；這不是 M4 candidate disposition 或 gate relaxation。 |
