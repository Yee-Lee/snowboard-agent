...
| --- | --- | --- |
| [Audio POC A / B 分組觀察計畫](Audio_POC_AB_experiement.md) | `RECEIVED / DATA PACKET PENDING` | 既有 per-item evidence 可重算；人工 fixture-quality sensitivity 已另記但不改 frozen scorecard。仍須提交 handoff 要求的 40-record sanitized data、summary、README 與正式 response。 |
| [DELIVERY-AUDIO-POC-M4A-M2AB-SCOPE-ACK-003](DELIVERY-AUDIO-POC-M4A-M2AB-SCOPE-ACK-003.md) | `ACCEPTED / M2A COMPLETE / M2B IN PROGRESS` | M2A common scorecard 與 shortlist 已 review；M2B base Q5→Q8 probe 已 review，後續單變因 probes、primary/fallback 與 exact recipe 尚未完成。VAD real-engine row 仍未授權。 |
