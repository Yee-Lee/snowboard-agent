...
| --- | --- | --- |
| [M3 packet sign-off request](../../poc_audio/deliveries/REQ-AUDIO-M3-PACKET-SIGNOFF-001.md) | `DELIVERING ONE-TIME ACK REQUEST / NO EXECUTION AUTHORITY` | runner candidate 固定為 `655e80ec4ed287708ed0a47f383b645d88650b18`，packet manifest SHA-256 為 `ebadd620...65c55`；Core 只需一次 ACK，不需改 code/test、驗 remote branch、產 runner/JSON 或操作 Pi。Audio 以 ACK commit SHA 建 controlled sign-off 後主導執行。 |
...
| [P9 executable correction](DELIVERY-012-PM-LLM-POC-P9-SURROGATE-EXECUTABLE.md) / [source identity](P9-SURROGATE-SOURCE-IDENTITY-001.md) | `ARTIFACT VERIFIED / CORE ACK PENDING / NON-BLOCKING FOR AUDIO M3` | 三個 locked checksum 吻合，附件 6-test regression 通過；舊 Core spec §3 example 不可執行，等待 Core corrected ACK 後再無語意變更地整合 exact artifact。不得宣稱 P9 PASS 或 LLM Gate 2 credit。 |
