...
兩次先前 runner-defect `INCONCLUSIVE` 永久保留。Core 的 transition ACK 已接受 ARM64
UTM為工程輸入、waive x86_64 track並固定 Gemma/Qwen 1.5B 為最多兩個 Pi inputs；這不產生
Gate 1/2 credit，下一個外部關卡仍是經審核的物理 Pi compatibility。
...
| **Gate 1** Candidate Proposal, environment preflight, pre-screen & Pi Compatibility | `IN_PROGRESS / BLOCKED` | POC Team 執行；Core Designer書面確認 | 兩個 frozen Pi inputs完成獨立產品Pi compatibility；只保留`PASS`且不補位 | 提交 `G1-PI-COMPAT-006` executable packet取得Core review/ACK，後於Pi執行 |
...
| **M2** | `IN_PROGRESS / BLOCKED` | ARM64 engineering pre-screen已接受；等待Pi compatibility packet ACK及實測，最多保留兩個 proposed finalists | [M2](m2_llm_candidate_evaluation.md) |
...
- **Accepted transition / current block**：Core已以
  `ACK-LLM-M2-ARM64-TO-PI-TRANSITION-001`接受ARM64工程輸入、x86 waiver與兩個candidate
  freeze；M2/Gate 1仍為`IN_PROGRESS / BLOCKED`，直到`G1-PI-COMPAT-006`由Core review後在
  Pi 5 4GB/Debian 13/swap=0實測並取得Gate 1 finalist ACK。P5正式15秒case只在Gate 2A Pi
  執行，workstation workaround維持`INCONCLUSIVE`。
...
- [Core ARM64 diagnostic and continuation ACK](../pm_handoff/history/ACK-LLM-M2-ARM64-PREFLIGHT-DIAGNOSTIC-001.md)
...
- [ARM64 diagnostic and bounded-continuation ACK](../pm_handoff/history/ACK-LLM-M2-ARM64-PREFLIGHT-DIAGNOSTIC-001.md)
