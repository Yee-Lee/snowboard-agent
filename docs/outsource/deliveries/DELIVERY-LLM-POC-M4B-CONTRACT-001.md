...
| **M4B-P9** | Gate 2B Combined Residency | prerequisite為Core記錄的Accepted Audio POC final handoff ID / SHA / kit；surrogate只可debug。4GB mandatory：swap=0，以`system_used = MemTotal - MemAvailable`為primary capacity metric且任一sample不得超3584 MiB；記錄Core parent + LLM + ASR/TTS process tree的PSS/RSS、threads、CPU、temperature、throttling及latency作diagnostic，sum RSS不得作capacity gate；不得OOM或增加full memory-pressure stall。8GB用相同設定跑informational sanity，不另放寬4GB結果 |
