...
| Base Python ABI | target-owned `/usr/bin/python3.13`；CPython `3.13.5`；SOABI `cpython-313-aarch64-linux-gnu`；MULTIARCH `aarch64-linux-gnu`；stdlib `/usr/lib/python3.13`；exact per-run ABI attestation仍是replacement design必備項目 |
...
payload manifest；replacement install、preflight與acceptance仍須對新設計的exact ABI attestation一致。
