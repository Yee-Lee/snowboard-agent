...
| D5 | Partially resolved / Pending immutable delivery | 定義 source/artifact checksum、toolchain、license、known limits、full SHA 與兩階段 acceptance。 | Contract §6、manifest、evidence index；目前工作樹未形成 immutable delivery SHA，`.so` checksum 與 Core integration SHA 待補。 |
...
| POC Display Team | 形成 clean immutable commit 後填入 full source SHA 及 artifact/header/adapter checksums。 |
