...
| D3 | Partially resolved / Hardware run ready | 改正 DC=BCM25/Board22、RST=BCM27/Board13；primary/optional fixture 分離；新增 strict local config、read-only pre-test、operator fixture/revision/owner gate；不要求照片。 | Contract §4、`poc_display/config/`、`poc_display/tools/`；operator attestation、resolved gpiochip 與實際 config hash 待 Pi packet。 |
