...
| D3 | Partially resolved / Hardware run retry pending | Primary fixture 固定為 co-I2S：DC=BCM24/Board18、RST=BCM25/Board22、CE0=BCM8/Board24（SPI kernel-managed，不可由 lgpio claim）；primary/optional fixture 分離；strict local config、read-only pre-test、operator fixture/revision/owner gate；不要求照片。 | Contract §4、`poc_display/config/`、`poc_display/tools/`；初次 P3 證實 CE0 重複 claim，config/runtime 修正後需在 Pi 重跑。 |
...
| D5 | Partially resolved / Candidate frozen; Pi materials pending | Tracked delivery 以 candidate full Git SHA 作單一提交包；只有無法上傳的 generated artifact、actual config 與 raw evidence 另記 checksum／custody。 | Candidate 為本次 freeze commit full SHA；`.so`、actual config、raw evidence 與 Core integration SHA 待後續 gate。 |
