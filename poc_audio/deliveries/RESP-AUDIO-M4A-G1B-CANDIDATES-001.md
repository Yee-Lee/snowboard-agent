...
- **Prepared**: 2026-08-18 amendment; supersedes the proposal at POC commit
  `426eccfa525ac3a8e1fb47f7b42c319930e7bcf4`
- **Architecture change**: `No`

## 1. Disposition and boundary

Gate 1A is accepted. Audio POC submits the exact candidate-scope proposal below
for Gate 1B review. This amendment adds the User-designated primary TTS
evaluation row `tts-sherpa-matcha-zh-en-1.13.5`; no earlier row or disposition
was removed or relaxed. The machine-readable authority is
...
| `tts-sherpa-matcha-zh-en-1.13.5` | TTS / alternative, User primary | sherpa-onnx `1.13.5`; Matcha zh/en archive `271b804…`; 16 kHz Vocos `b599142…`; exact two-wheel aarch64 closure | `AUTHORIZE` | GitHub asset digests, POC hashes and ModelScope commit `f05803e…` LFS OIDs agree. Model metadata says Apache-2.0, but the archive lacks a license file and training-data lineage needs Core legal review. |
...
5. `tts-sherpa-matcha-zh-en-1.13.5` — User-designated primary TTS evaluation row
6. `tts-sherpa-melo-zh-en-1.13.5`
...
`2026-08-17T14:45:29Z` and `2026-08-18T01:43:58Z`. Per-file UTC time, HTTPS URL,
...
bytes. The former Matcha `.partial` was not admitted until its full byte count,
bzip2 integrity, archive listing, POC hash and official release digest all
matched; it is now the complete controlled archive named in the manifest.
PocketSphinx’s 407-byte registry snapshot is negative provenance evidence, not a
speech model.
...
| sherpa-onnx | Apache-2.0 | SenseVoice lineage Apache-2.0; Paraformer unknown; Melo archive MIT; Matcha ModelScope metadata Apache-2.0 with no archive-embedded license and incomplete training-data notice | Exact runtime closure is `sherpa-onnx==1.13.5` CPython 3.13 aarch64 wheel plus `sherpa-onnx-core==1.13.5` aarch64 wheel; both POC-hashed. Matcha additionally fixes its acoustic archive and required 16 kHz Vocos artifact. |
...
voice and for final redistribution notices. Matcha may be authorized for isolated
evaluation because its exact bytes and declared Apache-2.0 metadata are fixed,
but it cannot become a final winner until its missing embedded notice and
unspecified mixed Chinese/English training-data lineage are accepted or resolved.
...
| sherpa Matcha zh/en | UTF-8 zh/en through embedded espeak-ng data, lexicon, tokens and Chinese rule FSTs | Archive and Vocos metadata declare 16000 Hz mono audio; sample representation and ordered S16_LE adapter boundary require post-ACK assertion. |
...
two for ASR and three for TTS, so the final baseline remains reachable without
...
2. lists all twelve row IDs as `ACCEPTED`, `REJECTED` or `DEFERRED` without
...
4. resolves whether the Whisper model notice, SenseVoice upstream-license copy,
   and Matcha pinned ModelScope license/training-data record are sufficient for
   POC evaluation; and
