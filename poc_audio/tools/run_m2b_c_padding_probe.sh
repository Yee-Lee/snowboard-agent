#!/usr/bin/env bash
# Run the C Internal dev-only endpoint-padding probe offline.
set -euo pipefail
...
if [[ -n "$(git -C "$repo_root" status --porcelain)" ]]; then
  echo "refusing C padding probe from a dirty POC worktree" >&2
  exit 1
fi
...
python3 -m audio_poc.m2b_c_padding_probe "$@"
