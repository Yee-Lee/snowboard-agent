...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "run with sudo: sudo bash poc_llm/tools/run_gate1_arm64_p5_workaround_batch.sh" >&2
  exit 2
fi
...
unshare --net --setuid "$operator_uid" --setgid "$operator_gid" -- bash -c '
  ...
  mkdir -p "$root"
  python3 poc_llm/tools/run_gate1_arm64_p5_workaround.py \
  ...
  echo "p5_workaround_qwen15_exit=$qwen_exit"
  echo "p5_workaround_gemma_exit=$gemma_exit"
  [[ "$qwen_exit" -eq 0 && "$gemma_exit" -eq 0 ]]
' arm64-p5-workaround "$execution_sha" "$lock_sha" "$raw_root" "$lock"
batch_exit="$?"
echo "p5_workaround_batch_exit=$batch_exit"
exit "$batch_exit"
