...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "run with sudo: sudo bash poc_llm/tools/run_gate1_arm64_long_p8_batch.sh" >&2
  exit 2
fi
...
raw_root="/tmp/llm-poc-g1-arm64-long-p8-001"
...
unshare --net --setuid "$operator_uid" --setgid "$operator_gid" -- bash -c '
  ...
  mkdir -p "$root"
  python3 poc_llm/tools/run_gate1_arm64_long_p8.py \
  ...
  echo "long_p8_qwen15_exit=$qwen_exit"
  echo "long_p8_gemma_exit=$gemma_exit"
  [[ "$qwen_exit" -eq 0 && "$gemma_exit" -eq 0 ]]
' arm64-long-p8 "$execution_sha" "$lock_sha" "$raw_root" "$lock"
batch_exit="$?"
echo "long_p8_batch_exit=$batch_exit"
exit "$batch_exit"
